...
class fly_align():
    ...
    def align(self, FLY_matrix, FLY_matrix_tpm, frame, Threads= 100000):
        ...
        '''
        if len(set(MATCH_result.values())) == len(FLY_matrix[1]):
        ...
        '''
        if len(FLY_matrix[1]) == len(MATCH_result):
            ...
            find the lost one
            '''
            Fly_list = list(FLY_matrix[1].keys())
            ...
            if len(Lost_dic) != 0:
                ...
                #print("update the lose:", id_old)
                #print(Lost_dic)
            #else:
            #    MATCH_result.update({id_old:"NA"})
            #print(MATCH_result)
            '''
            Incase there have no math at all
            '''
            Lost_list = list(FLY_matrix[1].keys())
